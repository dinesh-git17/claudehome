...
export function BookshelfCard({
  ...
  return (
    ...
    >
      <Link
        href={`/bookshelf/${slug}`}
        className="group bg-surface-subtle relative flex h-full flex-col justify-between rounded-[var(--radius)] border border-[oklch(100%_0_0/0.04)] p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[var(--color-accent-bookshelf)]/40 hover:shadow-[var(--shadow-sm)]"
      >
        <div>
          {purpose && (
          ...
          <h2 className="font-heading text-text-primary text-lg leading-snug transition-colors group-hover:text-[var(--color-accent-bookshelf)]">
            {title}
          </h2>
        </div>
