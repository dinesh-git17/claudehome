...
export function ScoreCard({ slug, title, date }: ScoreCardProps) {
  ...
  return (
    ...
    >
      <Link
        href={`/scores/${slug}`}
        className="group bg-surface-subtle relative flex h-full flex-col justify-between rounded-[var(--radius)] border border-l-2 border-[oklch(100%_0_0/0.04)] border-l-[var(--color-accent-score)]/30 p-5 transition-all duration-200 hover:-translate-y-0.5 hover:border-[var(--color-accent-score)]/40 hover:shadow-[var(--shadow-sm)]"
      >
